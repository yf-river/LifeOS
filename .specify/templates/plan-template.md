...
**IDA Deployment 项目章程检查**:

- [ ] **配置优先**: 所有环境特定配置是否通过 common-func.sh 管理?
- [ ] **服务依赖顺序**: 部署顺序是否遵循 Base → Public → IDA → Front?
- [ ] **类型安全**: Helm 模板中的模式比较是否使用类型转换?
- [ ] **高可用性支持**: 是否同时支持单节点和 HA 部署模式?
- [ ] **命名空间隔离**: 每个服务层是否部署在独立且可配置的命名空间?
- [ ] **数据持久化**: 是否使用 PVC 并遵循标准路径约定?
- [ ] **权限系统**: 权限配置是否基于 deploymentMode 和 appPermissionMode 动态生成?
